...
class ProgressivePlacerTest(object):
    ...
    def test(self, config):
        ...
        if graph == 'chain':
            ...
            # TODO 这个PPTFItem，提取有关于图特征，并且注入模拟器
            # 输入是
            pptf = PPTFItem(input_file, n_devices,
                            simplify_tf_reward_model=config['simplify_tf_rew_model'],
                            use_new_sim=config['use_new_sim'], # 是否使用新的模拟器
                            sim_mem_usage=True, # 模拟器是否采用内存衡量
                            # sim_mem_usage=False,
...
if __name__ == '__main__':
    ...
    # for synthetic non-tensorflow graphs 用tensorflow合成图的有关参数
