...
class ProgressivePlacer:
    ...
    def place(self, G, n_devs, nn_model, sim_eval, config, pptfitem=None):
        ...
        # TODO 此处进行了修改，dataset报错，因此替换成dataset_folder
        self.tb_dir = '%smodels/tb-logs/%s' % (config['model_folder_prefix'], config['dataset_folder'])
        self.eval_dir = '%smodels/eval-dir/%s' % (config['model_folder_prefix'], config['dataset_folder'])
        ...
        rnd_cum_rewards = [deque(maxlen=config['bl_n_rnds']) for _ in range(self.max_rounds)] # 312个节点，最大的reward都设置为1000
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            for ep in range(n_episodes):
                ...
                # 此处增加真实tensorflow的速度衡量
                generate_mg = self.pptfitem.generate_placement(ep_best_pl)
                item = gitem.Item(generate_mg, ignore_colocation=False, ignore_user_placement=False)
                _, true_run_time, _ = self.pptfitem.gcluster.MeasureCosts(item)
                ...
                with open(self.tb_dir + '/summary.txt', 'a+') as f:
                    ...
                    f.write('\nTrue Run Time: ' + str(true_run_time))
                    f.write('\n------------\n')
    ...
    def run_episode(self, sess, init_pl, nodes, is_eval_ep, ep, config):
        ...
        for pg in self.progressive_graphs:
            pg.reset_placement(init_pl) # 将所有的设备都设置成init_pl
            pg.new_episode() # 将所有的节点和当前节点的flag标记为unseen
